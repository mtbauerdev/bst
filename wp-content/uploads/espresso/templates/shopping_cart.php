...
<?php
		...
		foreach ( $result as $r ){
			...
			if (event_espresso_get_status($r->id) == 'ACTIVE') {
				...
				//CODE TO PULL SEAT AVAILABILITY FROM FILEMAKER 
				$categoryName = $r->category_name;
				switch ($categoryName) {
					case 'Mainstage':
						$stage = "Tickets_Mainstage";
						$layout = "Mainstage";
						break;
					case 'Family Series':
						$stage = "Tickets_FamilySeries";
						$layout = "Family%20Series";
						break;
					case 'B3 Series':
						$stage = "Tickets_B3";
						$layout = "B3%20Series";
				...
				$rawDate = $r->start_date; // ex. 2014-01-15
				$formattedDate = new DateTime($rawDate);
			 	$date = $formattedDate->format('m/d/Y');
			 	$time = time_to_24hr( event_espresso_get_time( $r->id ) );
				$status = "Open";
				$section = "G";
				$url = "http://216.31.246.83:8000/fmi/xml/fmresultset.xml?-db=" . $stage . "&-lay=" . $layout . "&Show%20Date=" . $date . "&Show%20Time=" . $time . "&Status=" . $status . "&Section=" . $section . "&Section.op=neq&-find";
				$ch = curl_init($url);
				...
				$xml = simplexml_load_string($response)
					or die("We're sorry...the ticketing server is offline for maintenance.<br />To purchase tickets please contact the box office at 916-443-5300.");
				$seatcount = $xml->resultset['count'];
				// END FILEMAKER CODE

		?>
				<div class="multi_reg_cart_block event-display-boxes ui-widget"  id ="multi_reg_cart_block-<?php echo $r->id ?>">
		
					<h3 class="event_title ui-widget-header ui-corner-top"><?php echo stripslashes_deep( $r->event_name ) ?> <span class="remove-cart-item" style="cursor:pointer"> <img class="ee_delete_item_from_cart" id="cart_link_<?php echo $r->id ?>" alt="Remove this item from your cart" src="<?php echo EVENT_ESPRESSO_PLUGINFULLURL ?>images/icons/remove.gif" /> </span> </h3>
							...
							<table id="cart-reg-details" class="event-display-tables">
								...
								<tbody>
									<tr>
										...
										<td>
											<?php echo event_espresso_get_time( $r->id ); ?>
										</td>
									</tr>
									<?php //echo "<tr><td colspan='2'>SEATS LEFT: " . $seatcount . "</td></tr>" ?>
									<?php if ( $seatcount < 21 ) { ?>
									<tr>
										<td colspan="2">
											<strong style="color:red">This show is almost full! Call 916-443-5300.</strong>&nbsp;&nbsp;<input type="button" value="Return to calendar&nbsp;&raquo;" class="cancel-order" id="cart_link_<?php echo $r->id ?>" />
										</td>
									</tr>
									<!-- <tr>
										<td colspan="2">
											<strong style="color:red">We're sorry...the ticketing server is offline for maintenance.<br />Please contact the box office at 916-443-5300.</strong><br /><input type="button" value="Remove order and return to calendar" class="cancel-order" id="cart_link_<?php echo $r->id ?>" />
										</td>
									</tr> -->
									<style>.price_list {display:none;}</style>
									<?php } ?>
									<tr>
										<td colspan="2">
...
//		printr( $_SESSION, '$_SESSION  <br /><span style="font-size:10px;font-weight:normal;">' . __FILE__ . '<br />line no: ' . __LINE__ . '</span>', 'auto' );
		...
		<div class="event-display-boxes ui-widget">
			...
			<div id="event_espresso_coupon_wrapper" class="clearfix event-data-display">
				<label class="coupon-code" for="event_espresso_coupon_code"><?php _e( 'Enter Coupon Code (USE ALL CAPS)', 'event_espresso' ); ?></label>
				<input type="text" 
			...
			<p id="event_espresso_submit_cart">
				<input type="submit" class="submit btn_event_form_submit ui-priority-primary ui-state-default ui-state-hover ui-state-focus ui-corner-all" name="Continue" id="event_espresso_continue_registration" value="<?php _e( 'Proceed', 'event_espresso' ); ?>&nbsp;&raquo;" />
			</p>
