...
if (!function_exists('event_espresso_invoke_cart_error')) {
	function event_espresso_invoke_cart_error( $events_in_session ) {
		...
		do_action('action_hook_espresso_log', __FILE__, __FUNCTION__, '');
		if (!is_array($events_in_session)) {
			echo __('It looks like you are attempting to refresh a page after completing your registration or your cart is empty. Please <a href="/calendar" title="Current and upcoming shows">go to the current and upcoming shows</a> page and try again.', 'event_espresso') . "<br />";
			return true;
		}
		return false;
	}
